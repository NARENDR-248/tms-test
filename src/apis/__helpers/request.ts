...
const tmsRequest = axios.create({
  baseURL: process.env.NODE_ENV === "development" ? "http://localhost:4000/v1/" : "http://localhost:4000/v1/",
  timeout: 60000,
  headers: {
...
tmsRequest.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    if (originalConfig && originalConfig?.url !== "/v1/auth/login" && error.response) {
